...
#from _pytest.assertion.util import string_types
...
class Event:
    def __init__(self, EventName, Year, StartValue, Counter, revision, Vehicles , tracks,track_name,simulation_name,result_file,result_file_name,counter):
    ...
    def SimulateEvent(self):    
            ...
            for counterVehicles in self.Vehicles:
                ...
                if('AutoX' in counterTracks):
                    ...
                    if ('Windows' in platform.platform()):
                        ...
                        cwd = cwd.replace('\\', '/')
                        file = cwd + '/CSV/'
                    cfile = open(os.getcwd()+"\\tracks\\"+self.track_name+"\\"+counterTracks, 'r')
                    ...
                    if ('Windows' in platform.platform()):
                        ...
                        cwd = cwd.replace('\\', '/')
                        file = cwd + '/CSV/'
                    cfile = open(os.getcwd()+"/tracks/"+self.track_name+"\\"+counterTracks, 'r')
                ...
                if('Skidpad' in counterTracks):
                    ...
                    if(isinstance(SkidpadScoring, string_types) == True): #if points are a string set points on 0
                        SkidpadScoring = 0
                        self.result_file.cell(row=self.counter, column=9).value = str(Tyour)
                        #Points.append('Skidpad: %s s, Score: %d' %(Tyour, SkidpadScoring))
                        #f.write('Skidpad|%f|-|%d|\n' %(Tyour, SkidpadScoring))
                    elif(isinstance(SkidpadScoring, string_types) == False):  #if points are not a string, go on
                        #Points.append('Skidpad: %s s, Score: %d' %(Tyour, SkidpadScoring))
                        self.result_file.cell(row=self.counter, column=9).value = str(Tyour)
                        #f.write('Skidpad|%f|-|%d|\n' %(Tyour, SkidpadScoring))
                    ...
                    if(isinstance(AccelScoring, string_types) == True):   #if points are a string set points on 0
                        AccelScoring = 0
                        self.result_file.cell(row=self.counter, column=11).value = str(Tyour)
                        #Points.append('Acceleration: %s s, Score: %d' %(Tyour, AccelScoring))
                        #f.write('Acceleration|%f|-|%d|\n' %(Tyour, AccelScoring))
                    elif(isinstance(AccelScoring, string_types) == False):    #if points are not a string, go on
                        #Points.append('Acceleration: %s s, Score: %d' %(Tyour, AccelScoring))
                        self.result_file.cell(row=self.counter, column=11).value = str(Tyour)
                        #f.write('Acceleration|%f|-|%d|\n' %(Tyour, AccelScoring))
                    ...
                    if(isinstance(AutoXScoring, string_types) == True):   #if points are a string set points on 0
                        AutoXScoring = 0
                        self.result_file.cell(row=self.counter, column=3).value = str(Tyour)
                        #Points.append('AutoX: %s s, Score: %d' %(Tyour, AutoXScoring))
                        #f.write('AutoX|%f|-|%d|\n' %(Tyour/60, AutoXScoring))
                    elif(isinstance(AutoXScoring, string_types) == False):    #if points are not a string, go on
                        #Points.append('AutoX: %s s, Score: %d' %(Tyour, AutoXScoring))
                        self.result_file.cell(row=self.counter, column=3).value = str(Tyour)
                        #f.write('AutoX|%f|-|%d|\n' %(Tyour/60, AutoXScoring))
                    ...
                    if(isinstance(EnduranceScoring, string_types) == True):   #if points are a string set points on 0
                        EnduranceScoring = 0
                        self.result_file.cell(row=self.counter, column=7).value = str(Tyour)
                        #Points.append('Endurance: %s s, Score: %d' %(Tyour, EnduranceScoring))
                        #f.write('Endurance|%f|-|%d|\n' %(Tyour/60, EnduranceScoring))
                    elif(isinstance(EnduranceScoring, string_types) == False):    #if points are not a string, go on
                        #Points.append('Endurance: %s s, Score: %d' %(Tyour, EnduranceScoring))
                        self.result_file.cell(row=self.counter, column=7).value = str(Tyour)
                        #f.write('Endurance|%f|-|%d|\n' %(Tyour/60, EnduranceScoring))
                    ...
                    if(isinstance(EfficiencyScoring, string_types) == True):  #if points are a string set points on 0
                        EfficiencyScoring = 0
                        self.result_file.cell(row=self.counter, column=5).value = str(Tyour)
                    ...
                    elif(isinstance(EfficiencyScoring, string_types) == False):   #if points are not a string, go on
                        #Points.append('Efficiency: %s s, Energy: %f kWh, Score: %d' %(Tyour, EyourTotal, EfficiencyScoring))
                        self.result_file.cell(row=self.counter, column=5).value = str(Tyour)
                        #f.write('Efficiency|%f|%f|%d|\n' %(Tyour/60, EyourTotal, EfficiencyScoring))
        ...
        self.result_file.cell(row=self.counter,column=2).value=str(AutoXScoring)
        self.result_file.cell(row=self.counter,column=4).value=str(EfficiencyScoring)
        self.result_file.cell(row=self.counter,column=6).value=str(EnduranceScoring)
        self.result_file.cell(row=self.counter,column=8).value=str(SkidpadScoring)
        self.result_file.cell(row=self.counter,column=10).value=str(AccelScoring)
